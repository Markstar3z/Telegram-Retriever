...
async def process_project_list(event, text: str):
    """Process a full project submission."""
    ...
    try:
        projects = mark_duplicates(
            pair_project_links(text)
        )

        if not projects:
            await safe_reply(
                event,
                "❌ I could not find any valid X project links.",
            )
            return
        ...
        for position, project in enumerate(projects, start=1):
            ...
            if project["is_duplicate"]:
                ...
                output_blocks.append(
                    ...
                    f"Status: ⚠️ Missing Telegram link\n"
                    f"Owner: Not checked\n"
                    f"Top active admins: Not checked"
                )
                ...
                output_blocks.append(
                    ...
                    f"Status: 🔒 Private invite link\n"
                    f"Owner: Not checked\n"
                    f"Top active admins: Private link unsupported"
                )

            else:
                tg_username = extract_public_tg_username(
                    tg_link
                )
                ...
                if not tg_username:
                    ...
                    try:
                        result = await get_priority_admins(
                            tg_link
                        )
                        ...
                        if owner or active_admins:
                            ...
                            output_blocks.append(
                                ...
                                f"TG link: {tg_link}\n"
                                f"Status: ⚠️ No public human admins\n"
                                f"Owner: Not publicly available\n"
                        ...
                        output_blocks.append(
                            ...
                            f"Error: Your Telegram account cannot "
                            f"access the administrator list"
                        )
                        ...
                        output_blocks.append(
                            ...
                            f"X link: {x_link}\n"
                            f"TG link: {tg_link}\n"
                            f"Status: 🔒 Invalid or expired "
                            f"private invitation"
                        )
